...
class Order extends DataObject {
	...
	function requireDefaultRecords() {
		...
		$exist = DB::fieldList("Order");
 		if(isset($exist['Shipping'])) {
 			if($orders = DataObject::get('Order')) {
				...
				if(method_exists('DB', 'alteration_message')) DB::alteration_message('The \'SimpleShippingModifier\' and \'TaxModifier\' objects have been successfully created and linked to the appropriate orders present in the \'Order\' table', 'created');	
 			}
 			...
 			if(method_exists('DB', 'alteration_message')) DB::alteration_message('The columns \'hasShippingCost\', \'Shipping\' and \'AddedTax\' of the table \'Order\' have been renamed successfully. Also, the columns have been renamed respectly to \'_obsolete_hasShippingCost\', \'_obsolete_Shipping\' and \'_obsolete_AddedTax\'', 'obsolete');
  		}
  		...
  		if($orders = DataObject::get('Order', "\"Status\" = 'Cancelled'")) {
  			...
  			if(method_exists('DB', 'alteration_message')) DB::alteration_message('The orders which status was \'Cancelled\' have been successfully changed to the status \'AdminCancelled\'', 'changed');
  		}
	}
