...
class CheckoutPage extends Page {
	...
	function requireDefaultRecords() {
		...
		if(!$page = DataObject::get_one('CheckoutPage')) {
			...
			if(method_exists('DB', 'alteration_message')) DB::alteration_message('Checkout page \'Checkout\' created', 'created');
		}
		...
		if($page->TermsPageID == 0 && $termsPage = DataObject::get_one('Page', "\"URLSegment\" = 'terms-and-conditions'")) {
			...
			if(method_exists('DB', 'alteration_message')) DB::alteration_message("Page '{$termsPage->Title}' linked to the Checkout page '{$page->Title}'", 'changed');
